...
cor.test(quad_flower_visit_date.status$minute, quad_flower_visit_date.status$visit_flower_hour, method = ("spearman"))
...
cor.test(quad_flower_visit_temp$Temp, quad_flower_visit_temp$visit_flower_hour, method = ('pearson'))
cor.test(quad_flower_visit_temp$Temp, quad_flower_visit_temp$visit_flower_hour, method = ('spearman'))

#By burn status

quad_flower_visit_temp.status <- quad_flower_visit_temp %>%
  filter(Status == 'B') 
#  filter(Status == 'M')
#  filter(Status == 'U')

cor.test(quad_flower_visit_temp.status$Temp, quad_flower_visit_temp.status$visit_flower_hour, method = ("pearson"))
cor.test(quad_flower_visit_temp.status$Temp, quad_flower_visit_temp.status$visit_flower_hour, method = ("spearman"))
rmcorr(GrpTyear, Temp, visit_flower_hour, quad_flower_visit_temp.status)

#using rmcorr for repeated measures
#for all measures
rmcorr(GrpTyear, Temp, visit_flower_hour, quad_flower_visit_temp)
...
quad_visit_sum_apis <- visits %>% ##total visits per quad per observation
  ...
  mutate(prop_visits = visits/total_visits) #proportion of apis visits out of total
